...
//const shortenCommand = require('./commands/shorten');
...
const channelInfo = {
    contextInfo: {
        ...
        forwardedNewsletterMessageInfo: {
            newsletterJid: '254113334497@s.whatsapp.net',
...
if (!isGroup) {
    // Check if the user's message contains a link
   
const linkRegex = /(https?:\/\/[^\s]+|www\.[^\s]+)/gi;

    if (linkRegex.test(userMessage)) {
        await sock.sendMessage(chatId, {
            text: "⚠️ *~NEGATIVE~* :\n*THE ACCOUNT OWNER DOESNT ACCEPT LINKS PLEASE AVOID SENDING THEM TO AVOID WHATSAPP BANS*."
        });
        return;
    }
}
...
/*function toBold(str) {
    ...
    return str.split("").map(c => {
        ...
        return i !== -1 ? bold[i] : c
          ...
          if (!isGroup && (userMessage === 'hi' || userMessage === 'hello' || userMessage === 'Yooo' || userMessage === 'Yoo' || userMessage === 'hey' || userMessage === 'bro'|| userMessage === 'Evening treasure'|| userMessage === 'Nduguu'|| userMessage === 'Yo'|| userMessage === 'Evening'|| userMessage === 'wozaa'|| userMessage === 'Good morning'|| userMessage === 'Good afternoon'|| userMessage === 'Good evening'|| userMessage === 'Hello 👋'|| userMessage === 'Aura')) {
              await sock.sendMessage(chatId, {
                  text:'*Hi*, 🙋🏽‍♂️ *am an assistant*.*Your message has been positively received please wait fo a feedback*, \n*Thankyou for your patence*...\n✨*~REGARDS~* : *Aura*',
...
if (userMessage.startsWith('.vcf')) {
    await vcfCommand.execute(sock, chatId, userMessage);
        ...
        if (userMessage.startsWith('.missedcall')) {
    await missedcallCommand.execute(sock, chatId, userMessage,senderId);
}
        sock.ev.on("call", async (callEvent) => {
  try {
    const callInfo = callEvent[0]; // first call event
    const from = callInfo.from;
    const callType = callInfo.isVideo ? "Video Call" : "Voice Call";
    const time = new Date();

    // Update missedcall.js (it will auto-reply if enabled)
    await missedCallCommand.updateMissedCall(sock, {
      from,
      callType,
      time,
    });

    console.log(`[MISSED CALL] From: ${from}, Type: ${callType}, Time: ${time}`);
  } catch (err) {
    console.error("Error handling missed call event:", err);
  }
});
    ...
    await anticallCommand.execute(sock, chatId, userMessage);
}  
  ...
  if (!userMessage.startsWith(".hibernate")) return;
    ...
    const args = userMessage.trim().split(/\s+/).slice(1); // get ["on"] or ["off"]
    ...
    await antibotCommand.execute(sock, message, args, isAdmin);
}*/  
        
        /*if (userMessage.startsWith('.scanvirus')) {
    await scanvirusCommand.execute(sock, chatId, message);
}*/
        ...
        if (isOwnerCommand) {
            if (!message.key.fromMe && !senderIsSudo) {
                await sock.sendMessage(chatId, { text: '⚠️This command is only available for the owner or sudo!' });
                return;
            }
        }
