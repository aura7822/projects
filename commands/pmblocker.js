...
function readState() {
    try {
        if (!fs.existsSync(PMBLOCKER_PATH)) return { enabled: false, message: '⚠️ `MESSAGE AUTOBLOCKED ...CLIENT CURRENTLY CANT RECEIVE PRIVATE MESSAGES`' };
        const raw = fs.readFileSync(PMBLOCKER_PATH, 'utf8');
        const data = JSON.parse(raw || '{}');
        return {
            enabled: !!data.enabled,
            message: typeof data.message === 'string' && data.message.trim() ? data.message : '⚠️ `MESSAGE AUTOBLOCKED ...CLIENT CURRENTLY CANT RECEIVE PRIVATE MESSAGES`'
        };
    } catch {
        return { enabled: false, message: '⚠️ `MESSAGE AUTOBLOCKED ...CLIENT CURRENTLY CANT RECEIVE PRIVATE MESSAGES`' };
    }
}
